  ...
  reducers: {
    ...
    addItem: (state, action: PayloadAction<CartItem>) => {
      if (state.cart) {
        ...
        state.cart.subtotal = state.cart.items.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0);
      }
    },
    updateItemQuantity: (state, action: PayloadAction<{ itemId: string; quantity: number }>) => {
      if (state.cart) {
        ...
        if (item) {
          ...
          state.cart.subtotal = state.cart.items.reduce((sum, i) => sum + i.unitPrice * i.quantity, 0);
        }
      }
    },
...
export const { setCart, addItem, updateItemQuantity, removeItem, applyCoupon, removeCoupon, clearCart, setLoading, setError } = cartSlice.actions;
...
export const selectCartDiscount = (state: { cart: CartState }) => state.cart.cart?.discountAmount ?? 0;
