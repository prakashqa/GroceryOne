/**
 * ManageCartsScreen Tests
 * TDD: Tests for backend fetch, pull-to-refresh, loading state, and date picker fix
 *
 * Bug: ManageCartsScreen never fetched from backend — only read Redux store.
 * Historical carts (yesterday, date picker) were invisible because Redux
...
import ManageCartsScreen from '../ManageCartsScreen';
...
describe('ManageCartsScreen', () => {
  ...
  describe('backend fetch and historical carts', () => {
    it('should sync backend carts to Redux when data arrives (including historical)', async () => {
      ...
      const { store } = renderWithProviders(<ManageCartsScreen />, {
        preloadedState: {
          ...tenantState,
          multiCart: {
            carts: [],
    ...
    it('should have RefreshControl on the cart list', () => {
      ...
      const { UNSAFE_getAllByType } = renderWithProviders(<ManageCartsScreen />, {
        preloadedState: {
          ...tenantState,
          multiCart: {
            carts: [
  ...
  describe('date picker dismiss fix', () => {
    it('should NOT change filter when date picker is dismissed', async () => {
      ...
      const { getByTestId, queryByTestId } = renderWithProviders(<ManageCartsScreen />, {
        preloadedState: {
          ...tenantState,
          multiCart: {
            carts: [
