/**
 * ActiveOrderPreview Component Tests
 * TDD tests for dashboard active order preview card
 */

import React from 'react';
import { render, fireEvent } from '@testing-library/react-native';
import { ActiveOrderPreview } from '../ActiveOrderPreview';
...
describe('ActiveOrderPreview', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders item count', () => {
      const { getAllByText } = render(<ActiveOrderPreview {...defaultProps} />);
    ...
    it('renders with testID', () => {
      const { getByTestId } = render(
        <ActiveOrderPreview {...defaultProps} testID="active-cart" />
      );
  ...
  describe('Optional Amount', () => {
    it('hides amount when totalAmount is undefined', () => {
      const { queryByText } = render(
        <ActiveOrderPreview {...defaultProps} totalAmount={undefined} />
      );
    ...
    it('hides amount when totalAmount is 0', () => {
      const { queryByTestId } = render(
        <ActiveOrderPreview {...defaultProps} totalAmount={0} testID="cart" />
      );
