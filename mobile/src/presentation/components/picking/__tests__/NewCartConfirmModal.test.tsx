/**
 * NewCartConfirmModal Component Tests
 */

import React from 'react';
import { render, fireEvent } from '@testing-library/react-native';
import NewCartConfirmModal from '../NewCartConfirmModal';
  ...
  useTranslation: () => ({
    t: (key: string, options?: { count?: number }) => {
      const translations: Record<string, string> = {
        'picking.newCartConfirmTitle': 'Start New Cart?',
        'picking.newCartConfirmMessage': `Your current cart has ${options?.count || 0} items. Starting a new cart will save this cart to your Cart List.`,
...
describe('NewCartConfirmModal', () => {
  ...
  it('renders when visible is true', () => {
    ...
    expect(getByText('Start New Cart?')).toBeTruthy();
  });
  ...
  it('does not render when visible is false', () => {
    ...
    expect(queryByText('Start New Cart?')).toBeNull();
  });
  ...
  it('displays correct item count in message', () => {
    ...
    expect(getByText(/Your current cart has 5 items/)).toBeTruthy();
  });
