...
describe('OrderListItem', () => {
  ...
  const renderOrderItem = (cartOverrides: Partial<ManagedCart> = {}, propOverrides: Record<string, any> = {}) => render(
    <OrderListItem
  ...
  it('renders cart name correctly', () => {
    expect(renderOrderItem().getByText('Morning Order')).toBeTruthy();
  });

  it('renders item count correctly', () => {
    expect(renderOrderItem().getByText('2 items')).toBeTruthy();
  });

  it('renders singular item text for single item', () => {
    expect(renderOrderItem({ items: [mockCart.items[0]] }).getByText('1 item')).toBeTruthy();
  });

  it('shows active indicator when isActive is true', () => {
    expect(renderOrderItem({}, { isActive: true, testID: 'cart-item' }).getByTestId('cart-item-active-indicator')).toBeTruthy();
  });

  it('does not show active indicator when isActive is false', () => {
    expect(renderOrderItem({}, { testID: 'cart-item' }).queryByTestId('cart-item-active-indicator')).toBeNull();
  });

  it('calls onPress with cart ID when pressed', () => {
    fireEvent.press(renderOrderItem({}, { testID: 'cart-item' }).getByTestId('cart-item'));
    expect(mockOnPress).toHaveBeenCalledWith('cart-1');
  });

  it('calls onLongPress with cart ID when long pressed', () => {
    const { getByTestId } = renderOrderItem({}, { testID: 'cart-item', onLongPress: mockOnLongPress });
    fireEvent(getByTestId('cart-item'), 'onLongPress');
    expect(mockOnLongPress).toHaveBeenCalledWith('cart-1');
  });

  it('renders item count badge', () => {
    expect(renderOrderItem({}, { testID: 'cart-item' }).getByTestId('cart-item-badge')).toBeTruthy();
  });

  it('renders chevron indicator', () => {
    expect(renderOrderItem({}, { testID: 'cart-item' }).getByTestId('cart-item-chevron')).toBeTruthy();
  });

  it('renders empty cart correctly', () => {
    expect(renderOrderItem({ items: [] }).getByText('0 items')).toBeTruthy();
  });

  it('renders cart icon', () => {
    expect(renderOrderItem({}, { testID: 'cart-item' }).getByTestId('cart-item-icon')).toBeTruthy();
  });

  describe('Order Name Visibility', () => {
    it('should have minWidth on cart name to prevent excessive truncation on mobile', () => {
      const style = flattenStyle(renderOrderItem({}, { testID: 'cart-item' }).getByText('Morning Order').props.style);
      expect(style.minWidth).toBeGreaterThanOrEqual(60);
    });

    it('should render long cart names without excessive truncation', () => {
      expect(renderOrderItem({ name: 'Test Long Cart Name' }).getByText('Test Long Cart Name')).toBeTruthy();
    });
  });

  describe('Default Order Name Translation', () => {
    it.each([
      ...
      expect(renderOrderItem({ name }).getByText('Default Cart')).toBeTruthy();
    });

    it('should NOT translate user-provided cart names', () => {
      expect(renderOrderItem({ name: 'My Custom Cart' }).getByText('My Custom Cart')).toBeTruthy();
    });
  });

  describe('Tablet Layout Support', () => {
    it('renders with flex sizing when numColumns is provided', () => {
      const { getByTestId } = renderOrderItem({}, { testID: 'cart-item', numColumns: 2 });
    ...
    it('renders with default margins when numColumns is 1 or not provided', () => {
      const { getByTestId } = renderOrderItem({}, { testID: 'cart-item', numColumns: 1 });
    ...
    it('renders with default margins when numColumns is undefined', () => {
      const { getByTestId } = renderOrderItem({}, { testID: 'cart-item' });
      expect(flattenStyle(getByTestId('cart-item-animated-container').props.style).marginHorizontal).toBe(0);
    });
  });

  describe('Paid Order Display', () => {
    it('should display paidAmount for paid cart with items', () => {
      ...
      expect(renderOrderItem(createPaidCart({ items: paidItems }), { testID: 'cart-item' }).getByTestId('cart-item-total')).toBeTruthy();
    });

    it('should display paidAmount for paid cart with empty items array', () => {
      expect(renderOrderItem(createPaidCart({ id: 'cart-paid-2', name: 'Paid Cart Empty', items: [], paidAmount: 350 }), { testID: 'cart-item' }).getByTestId('cart-item-total')).toBeTruthy();
    });

    it('should display paidItemCount for paid cart with empty items array', () => {
      expect(renderOrderItem(createPaidCart({ id: 'cart-paid-count', name: 'Paid Cart Count', items: [], paidAmount: 500, paidItemCount: 4 })).getByText('4 items')).toBeTruthy();
    });
    ...
    it('should NOT display price section for unpaid cart with no priced items', () => {
      ...
      expect(renderOrderItem({ id: 'cart-draft-1', name: 'Draft Cart', items: draftItems, status: 'draft' as const }, { testID: 'cart-item' }).queryByTestId('cart-item-total')).toBeNull();
    });
  });
});
